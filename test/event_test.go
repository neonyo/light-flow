...
func (e *errorSave) ListCheckpoints(recoveryId string) ([]flow.CheckPoint, error) {
	...
	cps, err := e.persist.ListCheckpoints(recoveryId)
	if err != nil {
		return nil, err
	}
	for i, wrap := range cps {
		cp := wrap.(Checkpoints)
		cp.Snapshot = []byte("error")
		cps[i] = cp
	}
	return cps, err
}
...
func flexErrorCheck(t *testing.T, stage flow.EventStage, location string, checks ...map[string]string) func(event flow.FlexEvent) bool {
	...
	return func(event flow.FlexEvent) bool {
		...
		t.Logf("Event[ %s ] [Level: %s] end", event.Name(), event.Level())
...
func TestCallbackErrorEvent(t *testing.T) {
	...
	proc.CustomStep(Fx[flow.Step](t).Inc().Step(), "1")
...
func TestSuspendEncryptErrorEvent(t *testing.T) {
	...
	process.CustomStep(Fx[flow.Step](t).SetCtx().Recover().Step(), "1")
	...
	resetEventEnv()
	flow.SetEncryptor(&errorEncryptor{encryptFailed: true})
	flow.EventHandler().Handle(flow.InSuspend, flexErrorCheck(t, flow.InSuspend, "Encrypt"))
	wf = flow.RegisterFlow("TestEncryptErrorEvent0")
	wf.EnableRecover()
	process = wf.Process("TestEncryptErrorEvent0")
	process.CustomStep(Fx[flow.Step](t).Inc().Recover().Step(), "1")
	ff = flow.DoneFlow("TestEncryptErrorEvent0", map[string]any{"password": "hello world"})
	waitCurrent(3)
	CheckResult(t, 3, flow.Error)(any(ff).(flow.WorkFlow))
}
...
func TestSuspendSerializeErrorEvent(t *testing.T) {
	...
	resetEventEnv()
	flow.EventHandler().Handle(flow.InSuspend, serializeErrorHandler(t))
	wf = flow.RegisterFlow("TestSuspendSerializeErrorEvent0")
	wf.EnableRecover()
	process = wf.Process("TestSuspendSerializeErrorEvent0")
	process.CustomStep(func(ctx flow.Step) (any, error) {
		return nil, fmt.Errorf("error")
	}, "1")
	ff = flow.DoneFlow("TestSuspendSerializeErrorEvent0", map[string]any{"foo": noSerializable{"1", "2"}})
	waitCurrent(1)
	CheckResult(t, 1, flow.Error)(any(ff).(flow.WorkFlow))
}
...
func TestRecoverPersistErrorEvent(t *testing.T) {
	...
	proc.CustomStep(Fx[flow.Step](t).SetCtx().Recover().Step(), "1")
	...
	resetEventEnv()
	flow.SuspendPersist(&errorSave{persist: &persisitImpl{}, readError: true})
	flow.EventHandler().Handle(flow.InRecover, flexErrorCheck(t, flow.InRecover, "Deserialize"))
	wf = flow.RegisterFlow("TestRecoverPersistErrorEvent2")
	wf.EnableRecover()
	proc = wf.Process("TestRecoverPersistErrorEvent2")
	proc.CustomStep(Fx[flow.Step](t).SetCtx().Recover().Step(), "1")
	ff = flow.DoneFlow("TestRecoverPersistErrorEvent2", nil)
	CheckResult(t, 2, flow.Error)(any(ff).(flow.WorkFlow))
	_, err = ff.Recover()
	if err == nil {
		t.Errorf("Expected error, got nil")
...
func TestResourceEvent(t *testing.T) {
	...
	proc.CustomStep(Fx[flow.Step](t).Attach("TestResourceEvent0", nil, nil).Step(), "1")
	...
	proc.CustomStep(Fx[flow.Step](t).Attach("TestResourceEvent1", nil, nil).Step(), "1")
	...
	proc.CustomStep(Fx[flow.Step](t).Attach("TestResourceEvent2", nil, nil).Step(), "1")
	...
	proc.CustomStep(Fx[flow.Step](t).Attach("TestResourceEvent3", nil, nil).Step(), "1")
	...
	proc.CustomStep(Fx[flow.Step](t).Attach("TestResourceEvent4", nil, nil).Step(), "1")
	proc.CustomStep(Fx[flow.Step](t).Recover().Step(), "2")
	...
	proc.CustomStep(Fx[flow.Step](t).Attach("TestResourceEvent5", nil, nil).Step(), "1")
	proc.CustomStep(Fx[flow.Step](t).Recover().Step(), "2", "1")
	...
	proc.CustomStep(Fx[flow.Step](t).Attach("TestResourceEvent6", nil, nil).Step(), "1")
	proc.CustomStep(Fx[flow.Step](t).Recover().Step(), "2", "1")
	...
	proc.CustomStep(Fx[flow.Step](t).Attach("TestResourceEvent7", nil, nil).Step(), "1")
	proc.CustomStep(Fx[flow.Step](t).Recover().Step(), "2", "1")
